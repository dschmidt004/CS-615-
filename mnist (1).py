...
with open('mnist_train.csv', newline='') as csvfile:
    fullDataSet = list(csv.reader(csvfile, delimiter=','))

validDataSet = []
with open('mnist_test.csv', newline='') as csvfile:
    validDataSet = list(csv.reader(csvfile, delimiter=','))
...
class FullyConnectedLayer ( Layer ) :
    ...
    def backwardADAMGrad(self, t, gradIn, biasIn, beta1, beta2 , eta):
        djdb = biasIn
        djdw = gradIn
        epsilon = 1e-8
        self.momentumDW = beta1*self.momentumDW + (1-beta1)*djdw
...
class Conv():
    ...
    def gradient(self, prev_grad):
        ...
        prev_grad_flat = prev_grad.transpose(1,2,3,0).reshape(self.shape_out[1], -1)

        self.w_grad += np.dot(prev_grad_flat, self.flat_x).reshape(self.weight.shape)
    ...
    def _flatten_conv(self, x, stride,h_out=None, w_out=None):  
        ...
        for h in range(0, h_out):
            ...
            for w in range(0, w_out):
                ...
                flatten_x[step_size:step_size+x.shape[0],:] =x[:, :,h*stride:h1,w*stride:h2].reshape(x.shape[0],-1)
        return flatten_x
    ...
    def _transposed_conv(self, x): 
        if self.stride != 1:
            ...
            stride1 = np.zeros((*x.shape[:2], stride_h, stride_w))
            for h in range(x.shape[2]):
                for w in range(x.shape[3]):
                    stride1[:, :, h*self.stride, w*self.stride] = x[:, :, h, w]
            x = stride1
        ...
        trans_conv = trans_conv_flat.T.reshape(*self.shape_in[1:],self.shape_in[0])
...
class TransposedConv():
    ...
    def forward(self, x):
        ...
        if self.stride != 1:
            ...
            for h in range(x.shape[2]):
                for w in range(x.shape[3]):
                    stride1[:, :, h*self.stride, w*self.stride] = x[:, :, h, w]
            x = stride1[:,:,:-self.stride+1,:-self.stride+1]
    ...
    def _conv(self, prev_grad):
        if self.padding != 0:
            ...
            grad_pad[:, :,self.padding:-self.padding,self.padding:-self.padding] = prev_grad
            prev_grad = grad_pad
...
class Discriminator():
    def __init__ (self):  
        ...
        self.L3 = FullyConnectedLayer(784, 1)
    ...
    def discriminateTrain(self, t, gradientLoss):
        ...
        l3Out = l3Out.reshape((1, 1, 28, 28))
        l2gradient = self.L2.gradient()
        l2Out = l3Out*l2gradient
        self.L1.gradient(l2Out)
        updateGrad = self.L1.w_grad
        self.L1.backwardADAM(t, updateGrad, 0.99, 0.999 , 0.0001)  
...
class Generator():
    ...
    def generateTrainMSE(self, t, gradientLoss):
        ...
        updateGrad = self.L5.w_grad
        self.L5.backwardADAM(t, updateGrad, 0.99, 0.999 , 0.0001)
        
        l4gradient = self.L4.gradient()
        l4Out = l5Out*l4gradient
        
        l3Out = self.L3.gradient(l4Out)
        updateGrad = self.L3.w_grad
        self.L3.backwardADAM(t, updateGrad, 0.99, 0.999 , 0.0001)
        
        #l2gradient = self.L2.gradient()
        #l2Out = l3Out*l2gradient
        
        #l2OutFlat = np.ndarray.flatten(l2Out)
        #self.L1.backwardADAM(t, l2OutFlat, 0.99, 0.999 , 0.001) 
        
    def generateTrainMSEAvg(self, gradientLoss):
        ...
        l2OutFlat = np.ndarray.flatten(l2Out)
        self.L1.backwardNoGrad(l2OutFlat) 
    
    def updateWeights(self, t, batchSize):
        updateGrad = self.L5.w_grad / batchSize
        self.L5.backwardADAMGrad(t, updateGrad, 0.99, 0.999 , 0.001)
        self.L5._reset_gradients()
        
        updateGrad = self.L3.w_grad / batchSize
        self.L3.backwardADAMGrad(t, updateGrad, 0.99, 0.999 , 0.001)
        self.L3._reset_gradients()
        
        updateGrad = self.L1.w_grad / batchSize
        updateBias = self.L1.b_grad / batchSize
        self.L1.backwardADAMGrad(t, updateGrad, updateBias, 0.99, 0.999 , 0.001)
        self.L1.resetGradients()
        
    def generateTrainGAN(self, t, gradientLoss):
        l6Out = self.L6.gradient()
        gradientLoss = l6Out * gradientLoss
        l5Out = self.L5.gradient(gradientLoss)
        updateGrad = self.L5.w_grad
...
for i in range(epoch):
    ...
    yHat = None
    for num in fullDataSet: 
        hrn = num[1:]
        hrn = np.array(hrn)
        hrn = hrn.astype(float)
        img = hrn.reshape((28,28))
        yHat = img
        ...
        if random.getrandbits(1) or not Discriminator:
            real = False
            img = resizeImage(img,50)
            img = gen.generate(img)
        
            yHat = img[0][0]
        
        
        sigOut = dis.discriminate(np.array([[yHat]]))
        
        y = np.array([0])
        if real:
            y = np.array([1])
        
        disLoss += LL.eval(y, sigOut[0])
        disGrad = LL.gradient(y,sigOut)
        ...
        if isDiscriminator:
            dis.discriminateTrain(i+1, disGrad)
        
        else:
            disGrad = dis.discriminateNotTrain(disGrad)
            disGrad = disGrad * 10**-3
            
            mseLoss += np.sum(MSE.eval(hrn, np.ndarray.flatten(yHat)))
            grad = MSE.gradient(hrn,np.ndarray.flatten(yHat))
            grad = grad.reshape((28,28))
            gen.generateTrainMSE(i+1, np.array([[grad]]))
            gen.generateTrainMSE(i+1, np.array(disGrad))
            
        
    
    print(correct/len(fullDataSet))
        
    
    discriminatorLoss = disLoss/len(fullDataSet)

    if not isDiscriminator:
        vImg = validDataSet[453]
        
        vImg = vImg[1:]
        vImg = np.array(vImg)
        vImg = vImg.astype(float)
        vImg = vImg.reshape((28,28))
        vImg = resizeImage(vImg,50)
        vImg = gen.generate(vImg)
        vImg = vImg[0][0]
        cv2.imwrite('end_' +  "{0:0=4d}".format(i) + ".png" , vImg) 
        
        
    if not isDiscriminator:
        if discriminatorLoss > 10:
            print("swapping to Discriminator")
            isDiscriminator = not isDiscriminator
            dis = Discriminator()
            
    if isDiscriminator:
        if discriminatorLoss < 2:   
            print("swapping to Generator")
            isDiscriminator = not isDiscriminator
    
    
    totalDISLoss.append(disLoss/len(fullDataSet))


plt.plot(range(len(discriminatorLoss)), discriminatorLoss, label="upscaleLoss")   
plt.title("")
plt.xlabel("epochs")
